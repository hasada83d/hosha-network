...
def preprocess_original_links(ori_link):
    ori_link["id"] = ori_link["Drm0LinkID"].astype("int64")
    ori_link["s"] = (ori_link["Mesh"].astype(str) +
                     ori_link["DrmNode1ID"].astype(str).str.zfill(5)).astype("int64")
    ori_link["t"] = (ori_link["Mesh"].astype(str) +
                     ori_link["DrmNode2ID"].astype(str).str.zfill(5)).astype("int64")
    duplicate_mask = ori_link["id"].duplicated(keep=False)
    if duplicate_mask.any():
        ori_link.loc[duplicate_mask, "id"] = ori_link.loc[duplicate_mask, "id"] + 10000000000000000
...
def finalize_network(final_nodes, final_links, ori_links, ori_nodes):
    ...
    vehicle_mask = final_links["layer_id"] != 1
...
def export_final_network(final_nodes, final_links, config, suffix="raw"):
    ...
    final_links["geometry"] = geometries
